...
describe('Persistent Node Chat Server', function() {
    ...
    request({
      ...
      request({
        ...
        dbConnection.query(queryString, queryArgs, function(err, results) {
          ...
          // console.log('results from post test :' + JSON.stringify(results));
  ...
  it('Should output all messages from the DB', function(done) {
    ...
    var queryString = 'INSERT INTO messages (Messagetext, Roomname) VALUES (\'Men like you can never change!\', \'main\')';
    var queryArgs = [];
  ...
  it('Should output all users from the DB', function (done) {
    ...
    var queryString = 'INSERT INTO users (username) VALUES (\'Todd\')';
    var queryArgs = [];
    ...
    dbConnection.query(queryString, queryArgs, function (err) {
      ...
      request('http://127.0.0.1:3000/classes/users', function (error, response, body) {
        var messageLog = JSON.parse(body);
        expect(messageLog[0].username).to.equal('Todd');
        // expect(messageLog[0].Roomname).to.equal('main');
        done();
      });
    });
